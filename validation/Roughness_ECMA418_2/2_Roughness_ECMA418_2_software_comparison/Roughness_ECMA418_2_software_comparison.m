...
ySQAT = OUT.roughnessTDep(:,2);
label_title = [wav_file '.wav (Channel 2)'];
label_fig = [wav_file ' (Channel 2)' '_TDep_Roughness'];

il_plt_tDep(xRef, yRef, xSQAT, ySQAT, label_title, save_figs, label_fig);

% plot - combined binaural
yRef = ref_results.TDepCombBinaural(:,2);
ySQAT = OUT.roughnessTDepBin(:,1);
label_title = [wav_file '.wav (Combined binaural)'];
label_fig = [wav_file ' (Combined binaural)' '_TDep_Roughness'];

il_plt_tDep(xRef, yRef, xSQAT, ySQAT, label_title, save_figs, label_fig);

%% plot - avg specific roughness  
...
ch1_SQAT = OUT.roughness90Pc(1);
ch2_SQAT = OUT.roughness90Pc(2);
combBinaural_SQAT = OUT.roughness90PcBin(1);
...
ylabel( 'Specific roughness (asper_{HMS}/Bark_{HMS})' );
...
ylabel( 'Implementation-Reference (asper_{HMS}/Bark_{HMS})' );
...
ylim([0 round(max(single_values(1))+1)]);
...
ax.XLabel.String = 'Time (s)';
 ...
 h = colorbar;
 zString = 'Specific roughness (asper_{HMS}/Bark_{HMS})';
