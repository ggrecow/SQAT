...
# Checks
Four layers, from definitional to behavioural:
...
4. **Warning behaviour.** Terhardt's upper-slope fit, $\mathrm{steep} = -24 - 230/f + 0.2\,L$, is defined for negative values only. A component whose level exceeds $120 + 1150/f$ dB (about 121 dB at 1 kHz) makes it non-negative; the filterbank then clamps the slope to zero and the excitation stops decaying towards higher frequencies, so the value returned is an extrapolation outside the range over which the metric was validated (60 and 70 dB SPL in this folder). The 125 dB SPL signal must raise the warning `SQAT:FluctuationStrength:TerhardtSlopeClamped`, which names the offending component (123.1 dB at 1000 Hz here) and points at the `dBFS` calibration as the likely cause. The same warning must stay silent at 60 and 70 dB SPL. With `method = 1` the warning is raised once per 2 s frame; the identifier lets a user mute it with `warning('off', ...)`.
