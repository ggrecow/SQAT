...
# Origin of the toolbox

The development of SQAT was initiatted in 2019 by [Gil Felix Greco](https://www.linkedin.com/in/gil-felix-greco-363985101/) aiming at the sound quality (SQ) analysis of the environmental noise generated by virtual aircraft designs. In 2021, this preliminary version of SQAT, which works only with specific inputs provided by an aircraft noise prediction research software (PANAM-DLR), was first applied to analyze the SQ of a novel aircraft concept (see publication below). 
...
In 2022, after inspiring discussions during an acoustics conference in South Korea, [Alejandro Osses](https://www.linkedin.com/in/alejandro-osses-10039883/) and [Roberto Merino-Martínez](https://www.linkedin.com/in/roberto-merino-martinez/) joined the SQAT team in order to foster the development and release of an open-source version of the toolbox. The open-source version of SQAT, which works with any calibrated sound file as input, is freely available for use since May 2023 in this GitHub repository. 

The toolbox is expected to be carefully maintained by the SQAT team in the long-term, thus supporting its use as a reliable and readily accessible tool. Moreover, additional metrics are expected to be included in future releases as long as enough verification and documentation related to their implementations are available. With this strong requirement, we expect to include only implementations which are already mature enough for general use, and thus will not have to be updated after released. Although this shall allow for the backward compatibility between different releases, this may not be always the case as unforeseen issues may be reported by the users after a release. Thus, please always make sure to double check the description of the most updated release in case you are using different versions of the toolbox.       
